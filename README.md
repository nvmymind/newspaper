...
- **유튜브 구독 영상**: 초기화면 우측 버튼으로 새 창을 열어, Google 로그인 후 구독 중인 채널의 최신 영상을 썸네일·날짜별 정렬로 볼 수 있음 (선택 기능)
...
- **8000 포트에서만** 무한 로딩·멈춤이 반복되면, **작업 관리자**에서 남아 있는 **Python** 프로세스를 모두 종료한 뒤 서버를 다시 실행해 보세요. 이전에 켰던 서버가 완전히 안 닫혀 있으면 포트가 꼬일 수 있습니다.
